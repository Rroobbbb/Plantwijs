...
def ahn_from_point(lat: float, lon: float) -> Tuple[Optional[float], dict]:
    ...
    # 1) Probeer directe numerieke properties
    for k in ("GRAY_INDEX", "GRAYINDEX", "value", "Band1", "band_1"):
        if k in props and props[k] not in (None, ""):
            try:
                hoogte = float(props[k])
                break
...
def advies_geo(
    ...
    ahn_hoogte, props_ahn = ahn_from_point(lat, lon)
    ...
    ahn_bron = _WMSMETA.get("ahn", {}).get("title") or "PDOK AHN WMS"
    out = {
        "fgr": fgr,
        "bodem": bodem_val,
        ...
        "ahn_bron": ahn_bron if ahn_hoogte is not None else "onbekend",
        "advies": items,
